defmodule TripPlannerWeb.V1.Trips.ActivityControllerTest do
  ...
  describe "vote" do
    test "authenticated user, and can vote on the activity; trip exists; activity exists", %{conn: conn} do
      user = insert(:user)
      user_id = user.id
      trip = insert(:trip, user: user)
      trip_id = trip.id
      ...
      params = %{is_interested: false}

      conn =
        conn
        |> authenticate(user)
        |> post(Routes.trip_activity_path(conn, :vote, trip.id, activity.id, params))
      ...
      assert %{
               ...
               "start_date" => 1_664_848_505,
               "end_date" => 1_664_848_505,
               # TODO why is this a string "false" instead of just boolean false
               "is_interested" => "false",
               "user" => %{"id" => ^user_id},
               "trip_id" => ^trip_id
             } = json_response(conn, 200)
    ...
    test "authenticated user, and can update the activity; trip doesn't exist", %{conn: conn} do
      ...
      conn =
        conn
        |> authenticate(user)
        |> post(Routes.trip_activity_path(conn, :vote, trip_id, activity_id, params))
    ...
    test "unauthenticated user", %{conn: conn} do
      ...
      params = %{is_interested: false}
      conn = post(conn, Routes.trip_activity_path(conn, :vote, trip.id, activity.id, params))
